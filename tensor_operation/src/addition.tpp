...
namespace tinytorch{
    ...
    template <typename T>
    Tensor<T> Addition<T>::operator()(const std::vector<Tensor<T>> operands) const {
    ...
    template <typename T>
    Tensor<T> Addition<T>::backward(const size_t input_idx, const std::vector<Tensor<T>> operands) const {
