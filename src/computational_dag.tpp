...
template<typename T>
ComputationalDAGNode<T>::ComputationalDAGNode(TensorOperation<T> tensor_operation, std::vector<graph::NodeId> operand_node_id) : tensorOperation_(tensorOperation), operand_node_id_(operand_node_id) {
    size_t idx = 0;
...
std::vector<Tensor<T>> ComputationalDAG<T>::collectOperands(graph::NodeId node_id) const {
    ...
    if(id == entry_point_){
        ...
        const vector<NodeId>& operand_ids = get(node_id).operand_id;
    ...
    for(const graph::NodeId id : node_ids){
        ...
        for(const graph::NodeId successor_id : getSuccessors(id)){
            for(size_t i=0; i<operands.size(); i++){ 
                gradient_wrt_inputs_[i] += matmul<T>(node.tensorOperation_.backwardWRTInputs(operands, i), get(successor_id).gradient_wrt_inputs_[operand_idx[id]]); 
            }
            for(size_t i=0; i<weights.size(); i++){ 
                gradient_wrt_parameters_[i] += matmul<T>(node.tensorOperation_.backwardWRTParameters(operands, i), get(successor_id).gradient_wrt_inputs_[operand_idx[id]]); 
            }
        }
    }
}
