namespace tinytorch{
    ...
    template<typename T>
    void gradient_descent_iteration<T>(ComputationalDAG<T>& model, const Tensor<T>& data, const Tensor<T>& targets, const T learning_rate){
        ...
        for(graph::NodeId node_id : topo_order){
            for(size_t i = 0; i < model.get(node_id).operand_descriptor_.size(); ++i){
                ...
                if(operand_descriptor.operand_type_ == PARAMETER){
                    graph::ParameterId operand_parameter_id = operand_descriptor.id_.parameter_id_;
                    model.get(operand_parameter_id).result_ -= learning_rate * model.get(node_id).jacobi_[i];
                }
            }
        }
    }
    ...
    template<typename T>
    void gradient_descent<T>(ComputationalDAG<T>& model, const Tensor<T>& data, const Tensor<T>& targets, const T learning_rate, const size_t epochs){
